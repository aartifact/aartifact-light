...
Include.includes(
  ...
  var panel_out = new WidgetPanelsTabbed({containerId:"panels_tabbed", content:{
    ...
    /* breakdown: {
      id: "panel_out",
      click: function(){ 
        aartifact.library.propositions_user = []; // Discard any user-defined propositions.
        var result = JSCC.split_into_blocks_starting_with_unindented_line(ge('input_text').value, '@');
        var panels = new WidgetPanelList({containerId: "panel_out"});
        if (result.statements.length == 0)
          panels.addPanel(WidgetPanelsTabbedCollapsible({content:{messages: WidgetPanel('there is no input')}}));
        else
          StaggeredIterator.prototype.anonymousStart(10, 1, function(){return upd_step(result.statements, new Environment(), panels);});
      }
    }, */
    'library of propositions': {
      id: "panel_library",
      cache: true, // Only run click handler first time.
      click: function(){
        var panels = new WidgetPanelList({containerId: "panel_library"});
        panels.addPanel(WidgetTextFilter({keyboardHandler: "WidgetFilterableListTable.prototype.filter_global(this,'library_global');"}));
        aartifact.library.propositions.map(function(props){
          var tabbed = {};
          tabbed[props[0]] = WidgetFilterableListTable({itemClass:'library_global', items:
            props[1]
              .map(function(p){p.statement = true; p.logical_formulas_statements_mark(); return p;})
              .map(function(p){return {keywords: aartifact.expression.keywords(p), content: aartifact.presentation.formatted2('html', p)};}),
          });
          panels.addPanel(WidgetPanelsTabbedCollapsible({content: tabbed}));
        });
        aartifact.presentation.render_math_equations_html();
      }
    }
  }});
  ...
  $('#overlay').show();
  Include.includes('regimes/undergrad_instruction_linear_algebra/parser.js', function(){
  //JSCC.load('aartifact.parse', 'regimes/undergrad_instruction_linear_algebra/parser.jscc', function(){
    ...
    Include.include('regimes/undergrad_instruction_linear_algebra/propositions.js', function(){
      $('#overlay').hide();
